...
export default function TagsInput() {
  ...
  return (
    <div>
      <div onKeyDown={handleKeyDown} ref={tagsInput} className="border-2 bg-transparent dark:border-dark-subtle border-light-subtle px-2 h-10 rounded w-full text-white flex items-center space-x-2 overflow-x-auto custom-scroll-bar transition">
        {tags.map(t => <Tag onClick={() => removeTag(t)} key={t}>{t}</Tag>)}
        <input type="text" ref={input} onFocus={handleOnFocus} onBlur={handleOnBlur} placeholder="Tag One, Tag Two" value={tag} onChange={handleOnChange} className="h-full flex-grow bg-transparent outline-none dark:text-white" />
      </div>
    </div>
  )
}
